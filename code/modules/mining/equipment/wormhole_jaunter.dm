...
/obj/item/wormhole_jaunter/proc/chasm_react(mob/user)
	if(user.get_item_by_slot(SLOT_BELT) == src)
		to_chat(user, span_notice("My [name] activates, saving you from the chasm!"))
		SSblackbox.record_feedback("tally", "jaunter", 1, "Chasm") // chasm automatic activation
		activate(user, FALSE)
	else
		to_chat(user, span_danger("[src] is not attached to my belt, preventing it from saving you from the chasm. RIP."))
