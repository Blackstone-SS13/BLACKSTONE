...
/datum/nanite_program/triggered/comm/hallucination/set_extra_setting(user, setting)
	...
	if(setting == NES_HALLUCINATION_TYPE)
		...
		var/hal_type_choice = input("Choose the hallucination type", name) as null|anything in sort_list(possible_hallucinations)
		if(!hal_type_choice)
			return
		switch(hal_type_choice)
			...
			if("Battle")
				...
				var/hal_choice = input("Choose the hallucination battle type", name) as null|anything in sort_list(sound_list)
			...
			if("Sound")
				...
				var/hal_choice = input("Choose the hallucination sound", name) as null|anything in sort_list(sound_list)
				if(!hal_choice || hal_choice == "random")
					hal_details = null
				else
					hal_details = hal_choice
			...
			if("Station Message")
				...
				var/hal_choice = input("Choose the hallucination station message", name) as null|anything in sort_list(msg_list)
				if(!hal_choice || hal_choice == "random")
					hal_details = null
				else
					hal_details = hal_choice
			if("Health")
				...
				var/hal_choice = input("Choose the health status", name) as null|anything in sort_list(health_list)
				if(!hal_choice || hal_choice == "random")
					hal_details = null
				else
					switch(hal_choice)
			...
			if("Alert")
				...
				var/hal_choice = input("Choose the alert", name) as null|anything in sort_list(alert_list)
				if(!hal_choice || hal_choice == "random")
					hal_details = null
				else
					hal_details = hal_choice
