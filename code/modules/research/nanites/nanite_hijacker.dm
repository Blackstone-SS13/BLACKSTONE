...
/obj/item/nanite_hijacker/afterattack(atom/target, mob/user, etc)
	...
	if(isliving(target))
		...
		switch(success)
			if(NONE)
				to_chat(user, span_notice("I don't detect any nanites in [target]."))
			if(COMPONENT_PROGRAM_INSTALLED)
				to_chat(user, span_notice("I insert the currently loaded program into [target]'s nanites."))
			if(COMPONENT_PROGRAM_NOT_INSTALLED)
				to_chat(user, span_warning("I try to insert the currently loaded program into [target]'s nanites, but the installation fails."))
