...
/obj/item/reagent_containers/medigel/attack_self(mob/user)
	...
	to_chat(user, span_notice("I will now apply the medigel's contents in [squirt_mode ? "short bursts":"extended sprays"]. You'll now use [amount_per_transfer_from_this] units per use."))
...
/obj/item/reagent_containers/medigel/attack(mob/M, mob/user, def_zone)
	...
	if(M == user)
		...
		if(self_delay)
			...
			if(!reagents || !reagents.total_volume)
				return
		to_chat(M, span_notice("I [apply_method] myself with [src]."))
		...
		M.visible_message(span_danger("[user] attempts to [apply_method] [src] on [M]."), \
							span_danger("[user] attempts to [apply_method] [src] on you."))
		if(!do_mob(user, M))
			return
		if(!reagents || !reagents.total_volume)
			return
		M.visible_message(span_danger("[user] [apply_method]s [M] down with [src]."), \
							span_danger("[user] [apply_method]s you down with [src]."))
