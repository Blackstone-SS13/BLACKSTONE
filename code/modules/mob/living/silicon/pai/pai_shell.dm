...
/mob/living/silicon/pai/proc/fold_out(force = FALSE)
	...
	if(!canholo && !force)
		to_chat(src, span_warning("My master or another force has disabled my holochassis emitters!"))
		return FALSE
...
/mob/living/silicon/pai/proc/choose_chassis()
	if(!isturf(loc) && loc != card)
		to_chat(src, span_boldwarning("I can not change my holochassis composite while not on the ground or in my card!"))
		return FALSE
	...
	chassis = choice
	update_resting()
	to_chat(src, span_boldnotice("I switch my holochassis projection composite to [chassis]."))
...
/mob/living/silicon/pai/proc/toggle_integrated_light()
	if(!light_range)
		...
		to_chat(src, span_notice("I enable my integrated light."))
	else
		set_light(0)
		to_chat(src, span_notice("I disable my integrated light."))
