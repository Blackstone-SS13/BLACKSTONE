...
/mob/living/simple_animal/hostile/guardian/bomb/AltClickOn(atom/movable/A)
	if(!istype(A))
		return
	if(loc == summoner)
		to_chat(src, span_danger("<B>I must be manifested to create bombs!</B>"))
		return
	if(isobj(A) && Adjacent(A))
		if(bomb_cooldown <= world.time && !stat)
			...
			to_chat(src, span_danger("<B>My powers are on cooldown! You must wait 20 seconds between bombs.</B>"))
...
/mob/living/simple_animal/hostile/guardian/bomb/proc/display_examine(datum/source, mob/user, text)
	text += span_holoparasite("It glows with a strange <font color=\"[namedatum.colour]\">light</font>!")
