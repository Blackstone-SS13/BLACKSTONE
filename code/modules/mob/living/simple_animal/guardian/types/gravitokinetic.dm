...
/mob/living/simple_animal/hostile/guardian/gravitokinetic/AttackingTarget()
	...
	if(isliving(target) && target != src)
		...
		to_chat(target, span_danger("Everything feels really heavy!"))
...
/mob/living/simple_animal/hostile/guardian/gravitokinetic/AltClickOn(atom/A)
	if(isopenturf(A) && is_deployed() && stat != DEAD && in_range(src, A) && !incapacitated())
		var/turf/T = A
		if(isspaceturf(T))
			to_chat(src, span_warning("I cannot add gravity to space!"))
			return
		visible_message(span_danger("[src] slams their fist into the [T]!"), span_notice("I modify the gravity of the [T]."))
