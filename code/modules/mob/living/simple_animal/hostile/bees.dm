	...
	if(istype(I, /obj/item/reagent_containers/syringe))
		...
		if(S.reagents.has_reagent(/datum/reagent/royal_bee_jelly)) //checked twice, because I really don't want royal bee jelly to be duped
			if(S.reagents.has_reagent(/datum/reagent/royal_bee_jelly,5))
				...
				user.visible_message(span_notice("[user] injects [src] with royal bee jelly, causing it to split into two bees, MORE BEES!"),span_warning("I inject [src] with royal bee jelly, causing it to split into two bees, MORE BEES!"))
			else
			...
			if(R && S.reagents.has_reagent(R.type, 5))
				...
				user.visible_message(span_warning("[user] injects [src]'s genome with [R.name], mutating its DNA!"),span_warning("I inject [src]'s genome with [R.name], mutating its DNA!"))
				name = queen.name
			else
