	...
	if(. && !egg_lain && iscarbon(target) && !ismonkey(target))
		...
		if(C.stat == DEAD)
			if(HAS_TRAIT(C, TRAIT_XENO_HOST))
				to_chat(src, span_danger("A foreign presence repels us from this body. Perhaps we should try to infest another?"))
				return
			Infect(target)
			to_chat(src, span_danger("With our egg laid, our death approaches rapidly..."))
