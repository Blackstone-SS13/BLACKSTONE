...
/obj/item/gun/ballistic/revolver/detective/screwdriver_act(mob/living/user, obj/item/I)
	...
	if(magazine.caliber == "38")
		...
		if(I.use_tool(src, user, 30))
			if(magazine.ammo_count())
				to_chat(user, span_warning("I can't modify it!"))
				return TRUE
		...
		to_chat(user, span_notice("I begin to revert the modifications to [src]..."))
		if(magazine.ammo_count())
		...
		if(I.use_tool(src, user, 30))
			...
			fire_sound = 'sound/blank.ogg'
			desc = initial(desc)
			to_chat(user, span_notice("I remove the modifications on [src]. Now it will fire .38 rounds."))
	return TRUE
		...
		if(chambered)
			...
			if(AC.fire_casing(user, user))
				...
				if(zone == BODY_ZONE_HEAD || zone == BODY_ZONE_PRECISE_R_EYE || zone == BODY_ZONE_PRECISE_MOUTH)
					...
					user.visible_message(span_danger("[user.name] cowardly fires [src] at [user.p_their()] [affecting.name]!"), span_danger("I cowardly fire [src] at your [affecting.name]!"), span_hear("I hear a gunshot!"))
				chambered = null
				return
...
/obj/item/gun/ballistic/revolver/russian/proc/shoot_self(mob/living/carbon/human/user, affecting = BODY_ZONE_HEAD)
	...
	user.visible_message(span_danger("[user.name] fires [src] at [user.p_their()] head!"), span_danger("I fire [src] at your head!"), span_hear("I hear a gunshot!"))
...
/obj/item/gun/ballistic/revolver/russian/soul/shoot_self(mob/living/user)
	...
	if(!SS.transfer_soul("FORCE", user)) //Something went wrong
		qdel(SS)
		return
	user.visible_message(span_danger("[user.name]'s soul is captured by \the [src]!"), span_danger("You've lost the gamble! Your soul is forfeit!"))
...
/obj/item/gun/ballistic/revolver/reverse/can_trigger_gun(mob/living/user)
	...
	if(process_fire(user, user, FALSE, null, BODY_ZONE_HEAD))
		user.visible_message(span_warning("[user] somehow manages to shoot [user.p_them()]self in the face!"), span_danger("I somehow shoot myself in the face! How the hell?!"))
		user.emote("scream")
