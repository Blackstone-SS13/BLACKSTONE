...
/obj/structure/pipe_cleaner/proc/cut_pipe_cleaner(mob/user)
	user.visible_message(span_notice("[user] pulls up the pipe cleaner."), span_notice("I pull up the pipe cleaner."))
...
/obj/item/stack/pipe_cleaner_coil/proc/place_turf(turf/T, mob/user, dirnew)
	...
	if(!isturf(T) || !T.can_have_cabling())
		to_chat(user, span_warning("I can only lay pipe cleaners on a solid floor!"))
		return
	...
	if(get_dist(T,user) > 1) // Too far
		to_chat(user, span_warning("I can't lay pipe cleaner at a place that far away!"))
		return
...
/obj/item/stack/pipe_cleaner_coil/proc/pipe_cleaner_join(obj/structure/pipe_cleaner/C, mob/user, showerror = TRUE, forceddir)
	...
	if((C.d1 == dirn || C.d2 == dirn) && !forceddir)
		if(!U.can_have_cabling())						//checking if it's a plating or catwalk
			if (showerror)
				to_chat(user, span_warning("I can only lay pipe cleaners on catwalks and plating!"))
			return
		else
