...
/datum/module_picker/Topic(href, href_list)
	...
	if(A.stat == DEAD)
		to_chat(A, span_warning("I are already dead!"))
		return
...
/datum/action/innate/ai/nuke_station/proc/set_us_up_the_bomb(mob/living/owner)
	...
	to_chat(owner, span_danger("!!! UNAUTHORIZED SELF-DESTRUCT ACCESS !!!"))
...
/obj/machinery/doomsday_device/proc/detonate()
	...
	for(var/i in GLOB.mob_living_list)
		...
		if(issilicon(L))
			continue
		to_chat(L, span_danger("The blast wave from [src] tears you atom from atom!"))
		L.dust()
...
/datum/AI_Module/large/mecha_domination
	...
	cost = 30
	upgrade = TRUE
	unlock_text = span_notice("Virus package compiled. Select a target mech at any time. <b>I must remain on the station at all times. Loss of signal will result in total system lockout.</b>")
	unlock_sound = 'sound/blank.ogg'
...
/obj/effect/proc_holder/ranged_ai/overload_machine/InterceptClickOn(mob/living/caller, params, obj/machinery/target)
	...
	target.audible_message(span_danger("I hear a loud electrical buzzing sound coming from [target]!"))
...
/datum/action/innate/ai/place_transformer/Activate()
	...
	to_chat(owner, span_warning("I are no longer able to shunt your core to APCs."))
	adjust_uses(-1)
