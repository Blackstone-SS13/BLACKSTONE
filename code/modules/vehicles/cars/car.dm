...
/obj/vehicle/sealed/car/mob_try_exit(mob/M, mob/user, silent = FALSE)
	if(M == user && (occupants[M] & VEHICLE_CONTROL_KIDNAPPED))
		to_chat(user, span_notice("I push against the back of \the [src]'s trunk to try and get out."))
...
/obj/vehicle/sealed/car/attacked_by(obj/item/I, mob/living/user)
	if(!I.force)
		return
	if(occupants[user])
		to_chat(user, span_notice("My attack bounces off \the [src]'s padded interior."))
		return
	return ..()
