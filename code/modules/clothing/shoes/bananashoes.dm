	...
	if(on)
		if(bananium.get_material_amount(/datum/material/bananium) < 100)
			...
			if(!always_noslip)
				clothing_flags &= ~NOSLIP
			update_icon()
			to_chat(loc, span_warning("I ran out of bananium!"))
		else
...
/obj/item/clothing/shoes/clown_shoes/banana_shoes/attack_self(mob/user)
	...
	if(sheet_amount)
		to_chat(user, span_notice("I retrieve [sheet_amount] sheets of bananium from the prototype shoes."))
	else
		to_chat(user, span_warning("I cannot retrieve any bananium from the prototype shoes!"))
...
/obj/item/clothing/shoes/clown_shoes/banana_shoes/ui_action_click(mob/user)
	...
	if(bananium.get_material_amount(/datum/material/bananium))
		on = !on
		update_icon()
		to_chat(user, span_notice("I [on ? "activate" : "deactivate"] the prototype shoes."))
		if(!always_noslip)
			if(on)
				clothing_flags |= NOSLIP
			else
		...
		to_chat(user, span_warning("I need bananium to turn the prototype shoes on!"))
