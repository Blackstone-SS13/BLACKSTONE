...
/obj/structure/mecha_wreckage/welder_act(mob/living/user, obj/item/I)
	...
	if(salvage_num <= 0 || !length(welder_salvage))
		to_chat(user, span_notice("I don't see anything that can be cut with [I]!"))
		return
	if(!I.use_tool(src, user, 0, volume=50))
		return
	if(prob(30))
		to_chat(user, span_notice("I fail to salvage anything valuable from [src]!"))
		return
	var/type = pick(welder_salvage)
	var/N = new type(get_turf(user))
	user.visible_message(span_notice("[user] cuts [N] from [src]."), span_notice("I cut [N] from [src]."))
...
/obj/structure/mecha_wreckage/crowbar_act(mob/living/user, obj/item/I)
	...
	if(crowbar_salvage.len)
		...
		user.visible_message(span_notice("[user] pries [S] from [src]."), span_notice("I pry [S] from [src]."))
		crowbar_salvage -= S
		return
	to_chat(user, span_notice("I don't see anything that can be cut with [I]!"))
