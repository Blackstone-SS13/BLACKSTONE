...
/datum/action/innate/mecha/mech_cycle_equip/Activate()
	...
	if(!chassis.selected)
		...
		chassis.occupant_message(span_notice("I select [chassis.selected]."))
	...
	for(var/A in available_equipment)
		...
		if(A == chassis.selected)
			if(available_equipment.len == number)
				...
				chassis.occupant_message(span_notice("I switch to no equipment."))
				button_icon_state = "mech_cycle_equip_off"
			else
				chassis.selected = available_equipment[number+1]
				chassis.occupant_message(span_notice("I switch to [chassis.selected]."))
				button_icon_state = "mech_cycle_equip_on"
			send_byjax(chassis.occupant,"exosuit.browser","eq_list",chassis.get_equipment_list())
			UpdateButtonIcon()
			return
...
/datum/action/innate/mecha/mech_switch_damtype/Activate()
	...
	switch(chassis.damtype)
		if("tox")
			new_damtype = "brute"
			chassis.occupant_message(span_notice("My exosuit's hands form into fists."))
		if("brute")
			new_damtype = "fire"
			chassis.occupant_message(span_notice("A torch tip extends from my exosuit's hand, glowing red."))
		if("fire")
			new_damtype = "tox"
