...
/obj/structure/reflector/attackby(obj/item/W, mob/user, params)
	...
	if(W.tool_behaviour == TOOL_WRENCH)
		if(anchored)
			...
			return
		user.visible_message(span_notice("[user] starts to dismantle [src]."), span_notice("I start to dismantle [src]..."))
		...
		if(obj_integrity < max_integrity)
			...
			if(W.use_tool(src, user, 40, volume=40))
				...
				user.visible_message(span_notice("[user] has repaired [src]."), \
