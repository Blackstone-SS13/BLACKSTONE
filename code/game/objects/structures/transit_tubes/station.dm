...
/obj/structure/transit_tube/station/MouseDrop_T(obj/structure/c_transit_tube_pod/R, mob/user)
	...
	user.visible_message(span_notice("[user] inserts [R]."), span_notice("I insert [R]."))
	qdel(R)
				...
				if(!pod.moving && (pod.dir in tube_dirs))
					if(open_status == STATION_TUBE_CLOSED)
						...
						if(pod.contents.len && user.loc != pod)
							user.visible_message(span_notice("[user] starts emptying [pod]'s contents onto the floor."), span_notice("I start emptying [pod]'s contents onto the floor..."))
