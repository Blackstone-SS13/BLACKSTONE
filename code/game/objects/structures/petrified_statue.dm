...
/obj/structure/statue/petrified/New(loc, mob/living/L, statue_timer)
	...
	if(L)
		...
		L.visible_message(span_warning("[L]'s skin rapidly turns to marble!"), span_danger("My body freezes up! Can't... move... can't... think..."))
		L.forceMove(src)
	...
	if(istype(src.loc, /mob/living/simple_animal/hostile/statue))
		...
		if(S.mind)
			if(petrified_mob)
				...
				to_chat(petrified_mob, span_notice("I slowly come back to my senses. You are in control of myself again!"))
		qdel(S)
