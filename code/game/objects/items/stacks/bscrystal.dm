...
/obj/item/stack/ore/bluespace_crystal/attack_self(mob/user)
	user.visible_message(span_warning("[user] crushes [src]!"), span_danger("I crush [src]!"))
...
/obj/item/stack/sheet/bluespace_crystal/attack_self(mob/user)// to prevent the construction menu from ever happening
	to_chat(user, span_warning("I cannot crush the polycrystal in-hand, try breaking one off."))
...
/obj/item/stack/sheet/bluespace_crystal/attack_hand(mob/user)
	if(user.get_inactive_held_item() == src)
		...
		use(1)
		if(!amount)
			to_chat(user, span_notice("I break the final crystal off."))
		else
			to_chat(user, span_notice("I break off a crystal."))
	else
		..()
