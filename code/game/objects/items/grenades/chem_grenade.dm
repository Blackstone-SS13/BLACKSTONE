...
/obj/item/grenade/chem_grenade/examine(mob/user)
	...
	if(user.can_see_reagents())
		...
		if(beakers.len == 2 && beakers[1].name == beakers[2].name)
			. += span_notice("I see two [beakers[1].name]s inside the grenade.")
		else
			for(var/obj/item/reagent_containers/glass/G in beakers)
				. += span_notice("I see a [G.name] inside the grenade.")
...
/obj/item/grenade/chem_grenade/attackby(obj/item/I, mob/user, params)
	...
	if(I.tool_behaviour == TOOL_SCREWDRIVER)
		if(stage == GRENADE_WIRED)
			...
			to_chat(user, span_warning("I need to add a wire!"))
		return
...
/obj/item/grenade/chem_grenade/preprime(mob/user, delayoverride, msg = TRUE, volume = 60)
	...
	if(user)
		...
		if(msg)
			if(landminemode)
				to_chat(user, span_warning("I prime [src], activating its proximity sensor."))
			else
...
/obj/item/grenade/chem_grenade/adv_release/attackby(obj/item/I, mob/user, params)
	if(I.tool_behaviour == TOOL_MULTITOOL && !active)
		...
		if (newspread != null && user.canUseTopic(src, BE_CLOSE))
			...
			to_chat(user, span_notice("I set the time release to [unit_spread] units per detonation."))
