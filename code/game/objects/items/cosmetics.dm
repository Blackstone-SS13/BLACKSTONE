			...
			if(user.used_intent.type == INTENT_HELP)
				if(H.gender == MALE)
					...
					if(!get_location_accessible(H, location))
						...
						return
					user.visible_message(span_notice("[user] tries to change [H]'s facial hairstyle using [src]."), span_notice("I try to change [H]'s facial hairstyle using [src]."))
					if(new_style && do_after(user, 60, target = H))
						user.visible_message(span_notice("[user] successfully changes [H]'s facial hairstyle using [src]."), span_notice("I successfully change [H]'s facial hairstyle using [src]."))
