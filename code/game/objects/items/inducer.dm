...
/obj/item/inducer/proc/recharge(atom/movable/A, mob/user)
	...
	if(C)
		...
		if(C.charge >= C.maxcharge)
			...
			recharging = FALSE
			return TRUE
		user.visible_message(span_notice("[user] starts recharging [A] with [src]."), span_notice("I start recharging [A] with [src]."))
		...
		if(done_any) // Only show a message if we succeeded at least once
			user.visible_message(span_notice("[user] recharged [A]!"), span_notice("I recharged [A]!"))
		recharging = FALSE
		return TRUE
	recharging = FALSE
