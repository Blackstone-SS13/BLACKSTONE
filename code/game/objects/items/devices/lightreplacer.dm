...
/obj/item/lightreplacer/attackby(obj/item/W, mob/user, params)
	...
	if(istype(W, /obj/item/stack/sheet/glass))
		...
		if(uses >= max_uses)
			...
			AddUses(increment)
			to_chat(user, span_notice("I insert a piece of glass into \the [src.name]. You have [uses] light\s remaining."))
			return
		else
	...
	if(istype(W, /obj/item/shard))
		...
		to_chat(user, span_notice("I insert a shard of glass into \the [src]. You have [uses] light\s remaining."))
		qdel(W)
		return
	...
	if(istype(W, /obj/item/light))
		...
		if(L.status == 0) // LIGHT OKAY
			...
			if(!user.temporarilyRemoveItemFromInventory(W))
				return
			to_chat(user, span_notice("I insert [L] into \the [src]."))
			AddShards(1, user)
			qdel(L)
		return
...
/obj/item/lightreplacer/proc/ReplaceLight(obj/machinery/light/target, mob/living/U)
	...
	if(target.status != LIGHT_OK)
		if(CanUse(U))
			if(!Use(U))
				return
			to_chat(U, span_notice("I replace \the [target.fitting] with \the [src]."))
