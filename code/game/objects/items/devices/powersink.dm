	...
	if(I.tool_behaviour == TOOL_WRENCH)
		if(mode == DISCONNECTED)
			...
			if(isturf(T) && !T.intact)
				...
				if(!attached)
					...
					user.visible_message( \
						...
						span_notice("I bolt \the [src] into the floor and connect it to the cable."),
						span_hear("I hear some wires being connected to something."))
			else
				to_chat(user, span_warning("\The [src] must be placed over an exposed, powered cable node!"))
		else
			...
			user.visible_message( \
				...
				span_notice("I unbolt \the [src] from the floor and detach it from the cable."),
				span_hear("I hear some wires being disconnected from something."))
