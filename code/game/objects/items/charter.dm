...
/obj/item/station_charter/attack_self(mob/living/user)
	...
	if(response_timer_id)
		to_chat(user, span_warning("You're still waiting for approval from my employers about my proposed name change, it'd be best to wait for now."))
		return
	...
	if(standard_station_regex.Find(new_name))
		to_chat(user, span_notice("My name has been automatically approved."))
		rename_station(new_name, user.name, user.real_name, key_name(user))
		return

	to_chat(user, span_notice("My name has been sent to my employers for approval."))
