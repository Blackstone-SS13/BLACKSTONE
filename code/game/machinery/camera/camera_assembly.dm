...
/obj/structure/camera_assembly/examine(mob/user)
	...
	switch(state)
		if(STATE_WRENCHED)
			. += span_info("I can secure it in place with a <b>welder</b>, or removed with a <b>wrench</b>.")
		if(STATE_WELDED)
			. += span_info("I can add <b>wires</b> to it, or <b>unweld</b> it from the wall.")
		if(STATE_WIRED)
			if(has_upgrades)
				. += span_info("I can remove the contained upgrades with a <b>crowbar</b>.")
			. += span_info("I can complete it with a <b>screwdriver</b>, or <b>unwire</b> it to start removal.")
		if(STATE_FINISHED)
			. += span_boldwarning("I shouldn't be seeing this, tell a coder!")
	...
	switch(state)
		if(STATE_WRENCHED)
			if(W.tool_behaviour == TOOL_WELDER)
				if(weld(W, user))
					to_chat(user, span_notice("I weld [src] securely into place."))
					setAnchored(TRUE)
					state = STATE_WELDED
				return
		...
		if(STATE_WELDED)
			if(istype(W, /obj/item/stack/cable_coil))
				...
				if(C.use(2))
					to_chat(user, span_notice("I add wires to [src]."))
					state = STATE_WIRED
				else
				...
				if(weld(W, user))
					to_chat(user, span_notice("I unweld [src] from its place."))
		...
		if(STATE_WIRED)	// Upgrades!
			if(istype(W, /obj/item/stack/sheet/mineral/plasma)) //emp upgrade
				...
				to_chat(user, span_notice("I attach [W] into [src]'s inner circuits."))
				return
...
/obj/structure/camera_assembly/wrench_act(mob/user, obj/item/I)
	...
	if(state != STATE_WRENCHED)
		return
	I.play_tool_sound(src)
	to_chat(user, span_notice("I detach [src] from its place."))
