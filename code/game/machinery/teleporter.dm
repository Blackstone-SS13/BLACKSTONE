...
/obj/machinery/teleport/hub/Bumped(atom/movable/AM)
	if(is_centcom_level(z))
		to_chat(AM, span_warning("I can't use this here!"))
		return
	if(is_ready())
		teleport(AM)
			...
			if(!calibrated && prob(30 - ((accuracy) * 10))) //oh dear a problem
				if(ishuman(M))//don't remove people from the round randomly you jerks
					...
					if(human.dna && human.dna.species.id == "human")
						to_chat(M, span_hear("I hear a buzzing in your ears."))
...
/obj/machinery/teleport/station/attackby(obj/item/W, mob/user, params)
	if(W.tool_behaviour == TOOL_MULTITOOL)
		...
		if(panel_open)
			M.buffer = src
			to_chat(user, span_notice("I download the data to the [W.name]'s buffer."))
		else
		...
		if(panel_open)
			link_console_and_hub()
			to_chat(user, span_notice("I reconnect the station to nearby machinery."))
			return
	else
		return ..()
