...
/obj/machinery/fat_sucker/close_machine(mob/user)
	if(panel_open)
		to_chat(user, span_warning("I need to close the maintenance hatch first!"))
		return
	..()
...
/obj/machinery/fat_sucker/AltClick(mob/living/user)
	if(!user.canUseTopic(src, BE_CLOSE))
		return
	if(user == occupant)
		to_chat(user, span_warning("I can't reach the controls from inside!"))
		return
	if(!(obj_flags & EMAGGED) && !allowed(user))
		to_chat(user, span_warning("I lack the required access."))
		return
	free_exit = !free_exit
...
/obj/machinery/fat_sucker/emag_act(mob/living/user)
	...
	start_at = 100
	stop_at = 0
	to_chat(user, span_notice("I remove the access restrictions and lower the automatic ejection threshold!"))
	obj_flags |= EMAGGED
