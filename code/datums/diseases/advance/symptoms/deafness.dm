	...
	switch(A.stage)
		if(3, 4)
			if(prob(base_message_chance) && !suppress_warning)
				to_chat(M, span_warning("[pick("You hear a ringing in my ear.", "Your ears pop.")]"))
		if(5)
			if(power >= 2)
				var/obj/item/organ/ears/ears = M.getorganslot(ORGAN_SLOT_EARS)
				if(istype(ears) && ears.damage < ears.maxHealth)
					to_chat(M, span_danger("My ears pop painfully and start bleeding!"))
					ears.damage = max(ears.damage, ears.maxHealth)
					M.emote("scream")
			else
				to_chat(M, span_danger("My ears pop and begin ringing loudly!"))
