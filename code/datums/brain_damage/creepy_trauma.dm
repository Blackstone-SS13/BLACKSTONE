...
/datum/brain_trauma/special/obsessed/on_gain()
	...
	if(!obsession)//admins didn't set one
		...
		if(!obsession)//we didn't find one
			...
			qdel(src)
			return
	gain_text = span_warning("I hear a sickening, raspy voice in your head. It wants one small task of you...")
...
/datum/brain_trauma/special/obsessed/proc/social_interaction()
	...
	switch(rand(1,4))
		...
		if(2)
			...
			fail = TRUE
		if(3)
			to_chat(owner, span_danger("I feel your heart lurching in your chest..."))
			owner.Stun(20)
			shake_camera(owner, 15, 1)
		if(4)
